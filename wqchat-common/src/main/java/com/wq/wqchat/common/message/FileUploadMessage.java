...
public final class FileUploadMessage extends BaseMessage {

    public byte[] content;

    public FileUploadMessage(Packet packet, Connection connection) {
        super(packet, connection);
    }

    public static FileUploadMessage build(Connection connection) {
        if (connection.getSessionContext().isSecurity()) {
            return new FileUploadMessage(new Packet(PUSH, genSessionId()), connection);
        } else {
            return new FileUploadMessage(new JsonPacket(PUSH, genSessionId()), connection);
        }
    }
    ...
    public FileUploadMessage setContent(byte[] content) {
