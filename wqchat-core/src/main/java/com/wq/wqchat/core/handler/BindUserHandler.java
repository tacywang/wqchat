...
import com.wq.wqchat.api.spi.handler.BindValidator;
import com.wq.wqchat.api.spi.handler.BindValidatorFactory;
...
import com.wq.wqchat.common.message.BindUserMessage;
...
public final class BindUserHandler extends BaseMessageHandler<BindUserMessage> {
	...
	private PubService pubService = PubServiceFactory.create();
	...
	private void bind(BindUserMessage message) {
		...
		if (context.handshakeOk()) {
			...
			boolean success = validator.validate(message.userId)
					&& RouterCenter.I.register(message.userId, message.getConnection());
			if (success) {
				context.userId = message.userId;
				context.tags = message.tags;
				EventBus.I.post(new UserOnlineEvent(message.getConnection(), message.userId));
				OkMessage.from(message).setData("bind success").sendRaw();
				pubService.subMessage(message.userId);
			...
			Logs.CONN.error("bind user failure not handshake, userId={}, conn={}", message.userId,
					message.getConnection());
		}
	}

	/**
	 * 目前是以用户维度来存储路由信息的，所以在删除路由信息时要判断下是否是同一个设备 后续可以修改为按设备来存储路由信息。
	 *
	 * @param message
	 */
	private void unbind(BindUserMessage message) {
		if (Strings.isNullOrEmpty(message.userId)) {
			ErrorMessage.from(message).setReason("invalid param").close();
			Logs.CONN.error("unbind user failure invalid param, session={}",
					message.getConnection().getSessionContext());
			return;
		}
		// 1.解绑用户时先看下是否握手成功
		SessionContext context = message.getConnection().getSessionContext();
		if (context.handshakeOk()) {
			...
			if (localRouterManager.lookupAll(userId) == null || localRouterManager.lookupAll(userId).size() == 0) {
				pubService.unSubMessage(context.userId);
			...
			if (unRegisterSuccess) {
				...
				Logs.CONN.error("unbind user failure, unRegister router failure, userId={}, session={}", userId,
						context);
			}
		} else {
			ErrorMessage.from(message).setReason("not handshake").close();
